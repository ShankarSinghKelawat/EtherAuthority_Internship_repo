...
contract StakingContract is ReentrancyGuard{
    ...
    uint private totalStackedTokens;
    ...
    event ClaimReward(address indexed user, uint indexed amount);
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return rewardPerTokenStored +
        ((rewardRate * timeElapsed * PRECISION) / totalStackedTokens);
    }
    ...
    function withdraw(uint amount) external nonReentrant updateReward(msg.sender){
        require(amount > 0, "Amount must be greater than Zero");
        require(stakedBalance[msg.sender] >= amount, "Invalid amount to Withdraw");
        totalStackedTokens-=amount;
        stakedBalance[msg.sender]-=amount;
